...
def get_linearly_seperated_env():
    ...
    w_2 = np.array([-1, 1])
    return SwitchSystem(
        linear_systems=[LinearSystem(A_1, B_1, w_1), LinearSystem(A_2, B_2, w_2)],
        x=np.array([0, 1]),
    )
...
def get_2d_spiral(u_max: Optional[float] = None):
    ...
    return SwitchSystem(linear_systems=[linear, dummy])


def get_skewed_spiral(u_max: Optional[float] = None):
    A = np.array([[0.99257846, 0.99999352], [-0.0074215, 0.99999378]])
    B = np.array([[0.00150818], [0.00150817]])
    linear = LinearSystem(A=A, B=B, u_max=u_max)

    A_2 = np.array([[-1, 0], [0, -1]])
    b_2 = -100
    dummy = LinearSystem(A=A_2, B=B, b=b_2)
    return SwitchSystem(linear_systems=[linear, dummy])
